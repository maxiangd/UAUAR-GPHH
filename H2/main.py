...
def shortest_path_routing(graph, start, battery):
    ...
    # Create a graph copy to mark visited edges without changing the original graph
    ...
    # Initialization: calculate minimum cost from depot to all edges and build edge sets
    for node in graph:
        for neighbor, cost, demand, visit in graph[node]:
            # Calculate minimum cost from depot to the edge (node, neighbor)
            edge_cost = depot_to_path(graph_copy, start, node, neighbor)
            max_cost.append(edge_cost)
            # Standardize edges to eliminate duplicates (e.g. (2,3) and (3,2) are the same edge)
            std_head = min(node, neighbor)
            std_tail = max(node, neighbor)
            # Add edge to list without duplication
    ...
    # Process edge groups in ascending order of cost
    for mcost in max_cost:
        ...
        while not are_all_edges_true(edges):
            ...
            # Predict power consumption for task execution and return to depot
            if predicted_power_consumption < current_battery:
                # Step 1: Find the optimal unvisited edge under current cost
                ...
                # Exit loop if no valid unvisited edge exists (defensive check)
                ...
                if cost1 <= cost2:  # Prioritize moving to head node
                    ...
                    if not total_path:
                        ...
                        total_path.extend(path2[1:])  # Skip duplicate current node
                ...
                if current_location != start:
                    ...
                    current_battery = battery
        # All edges in current group have been processed

    # Return to the depot after all tasks are completed
    if current_location != start:
...
def monitoring_whether_return(graph, current_location, max_cost, max_path, start):
    """Select one optimal unvisited edge and return corresponding total cost"""
    total_cost1 = None  # Total cost for completing task and returning to depot
    total_cost2 = None  # Cost for completing the single task
    for mcost in max_cost:
        edges = max_path[mcost]
        current_min_unserved = float('inf')
        current_head = None
        ...
        # Step 1: Find the optimal unvisited edge (select only one)
        for i, (head, tail, cost, demand, visit) in enumerate(edges):
            if not visit:  # Only process unvisited edges
        ...
        # Process the selected edge if it exists
...
def min_cost(graph, path_list):
    ...
    for i in range(len(path_list) - 1):
        u = path_list[i]
        v = path_list[i + 1]
        # Search travel cost from node u to node v
...
def depot_to_path(graph, depot, path_start, path_end):
    """Calculate minimum cost from depot to the specified edge (path_start-path_end)"""
...
def floyd_shortest_with_path(graph, start, end):
    ...
    if start not in vertices or end not in vertices:
        return (None, None)  # Start or end node is not in the graph
    ...
    for u in graph:
        for neighbor_info in graph[u]:
            v, cost, _, _ = neighbor_info  # Parse neighbor information
            if cost < dist[u][v]:  # Ensure the minimum direct distance for duplicate edges
                dist[u][v] = cost
                prev[u][v] = u
    ...
    # Core iteration of Floyd-Warshall algorithm to update distance and predecessor
    for k in vertices:
        for i in vertices:
            for j in vertices:
...
def update_graph_with_random(graph, k, seed):
    ...
    """
    # Create independent random generator to avoid global state interference
    ...
    for node in updated_graph:
        ...
        for i in range(len(neighbors)):
            ...
            if edge not in processed_edges:
                # Generate new values via independent random generator
...
if __name__ == "__main__":
    ...
    for i in range(1, count + 1):
        ...
        # Regenerate graph if any edge contains zero value
