...
def eaSimple(population, toolbox, cxpb, mutpb, ngen, stats=None,
    ...
    for gen in range(1, ngen + 1):
        # 选父代最优的10个个体，clone避免引用传递
        elite = [toolbox.clone(ind) for ind in tools.selBest(population, k=ELITE_SIZE)]
        # Select the next generation individuals
        offspring = toolbox.select(population, len(population))
        offspring = [toolbox.clone(ind) for ind in offspring]  # 关键：clone避免引用
        ...
        try:
            # 提取每个子代的适应度值（FitnessMin是单目标，取values[0]）
            offspring_fitness = [ind.fitness.values[0] for ind in offspring]
            # 按适应度降序排序，获取索引（前10个是最差）
            sorted_worst_idxs = sorted(
                range(len(offspring)),
                key=lambda i: offspring_fitness[i],
                reverse=True
            )
            # 取前10个最差位置
            worst_idxs = sorted_worst_idxs[:ELITE_SIZE]
        except IndexError:
            # 极端情况：子代无有效适应度（理论上不会发生）
            worst_idxs = list(range(min(ELITE_SIZE, len(offspring))))

            # 步骤2：一一对应替换精英→最差位置
        for idx, (elite_ind, worst_idx) in enumerate(zip(elite, worst_idxs)):
            # 规范适应度比较（用values[0]，而非直接比较fitness对象）
            elite_fit = elite_ind.fitness.values[0]
            worst_fit = offspring[worst_idx].fitness.values[0]
            # 仅当精英比最差个体好时替换（FitnessMin：精英值更小）
            if elite_fit < worst_fit:
                offspring[worst_idx] = elite_ind

        # Update the hall of fame with the generated individuals
        if halloffame is not None:
            halloffame.update(offspring)
        print("Best:", halloffame[0])
